export default {
  id: 'P15-TC-16',
  title: 'Control panel diagnostics template contains graph/cache actions',
  phases: [15],
  kind: 'auto',
  expected: 'Template exposes graphInvalidate and graphRebuild actions',
  async run({ ctx, engine }) {
    const activeEngine = engine ?? ctx?.engine ?? game.janus7;
    // In Janus7 v0.9.11+, actions are data-driven via panel-registry.js
    const diags = activeEngine.uiLayer?.getQuickPanels?.().find(p => p.id === 'diagnostics')
                ?? activeEngine.uiLayer?.getPanel?.('diagnostics');
    
    if (!diags) {
       return { 
         ok: false, 
         summary: 'diagnostics panel missing in registry',
         details: `uiLayer keys: [${Object.keys(activeEngine.uiLayer || {}).join(', ')}]`
       };
    }

    const actions = diags?.actions || [];
    const hasInvalidate = actions.some(a => a.action === 'graphInvalidate');
    const hasRebuild = actions.some(a => a.action === 'graphRebuild');
    const ok = hasInvalidate && hasRebuild;
    
    return { 
      ok, 
      summary: ok ? 'diagnostics registry actions found' : 'graph diagnostics actions missing in registry',
      details: `Found actions: [${actions.map(a => a.action).join(', ')}]`
    };
  }
};
