export default {
  ...
  async run({ ctx }) {
    ...
    const { stateJson } = context;
    if (typeof stateJson !== 'string') throw new Error(`context.stateJson ist kein String, sondern: ${typeof stateJson}`);
    if (!stateJson.trim()) throw new Error('context.stateJson ist leer');

    let parsed;
    try {
      parsed = JSON.parse(stateJson);
    } catch (e) {
      throw new Error(`context.stateJson ist kein valides JSON: ${e.message}`);
    }
    ...
    if (missingKeys.length) {
      ...
      return {
        ok: true,
        summary: `JSON valid (${stateJson.length} Zeichen) | Hinweis: erwartete Keys fehlen: ${missingKeys.join(', ')}`
      };
    }

    return { ok: true, summary: `JSON valid | ${stateJson.length} Zeichen | Keys: ${Object.keys(parsed).join(', ')}` };
  }
};
